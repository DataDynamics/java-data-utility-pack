...
public class HdfsResourceLoader extends DefaultResourceLoader implements ResourcePatternResolver, PriorityOrdered, Closeable {
    ...
    private static final Logger logger = LoggerFactory.getLogger(HdfsResourceLoader.class);
    ...
    protected Set<Resource> doFindMatchingFileSystemResources(Path rootDir, String subPattern) throws IOException {
        if (logger.isDebugEnabled()) {
            logger.debug("Looking for matching resources in directory tree [" + rootDir.toUri().getPath() + "]");
        }
    ...
    protected Set<Path> retrieveMatchingFiles(Path rootDir, String pattern) throws IOException {
        ...
        if (!exists) {
            ...
            if (logger.isDebugEnabled()) {
                logger.debug("Skipping [" + rootDir.toUri().getPath() + "] because it does not exist");
            }
            return Collections.emptySet();
        }
        ...
        if (!fileStatus.isDir()) {
            ...
            if (logger.isWarnEnabled()) {
                logger.warn("Skipping [" + rootDir.toUri().getPath() + "] because it does not denote a directory");
            }
            return Collections.emptySet();
        }
    ...
    protected void doRetrieveMatchingFiles(String fullPattern, Path dir, Set<Path> result) throws IOException {
        if (logger.isDebugEnabled()) {
            logger.debug("Searching directory [" + dir.toUri().getPath() +
        ...
        if (dirContents == null) {
            if (logger.isWarnEnabled()) {
                logger.warn("Could not retrieve contents of directory [" + dir.toUri().getPath() + "]");
            }
            return;
        }
